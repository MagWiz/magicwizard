...

{
     ...
     */
     ...
     while ( window.isOpen() ) {
          ...
          sf::Time timeFPSTmp = Clock.getElapsedTime();
          deltaTime = Clock.restart().asSeconds();
          ...
          if ( hud.lastFPSUpdate.getElapsedTime().asSeconds() >0.2f ) {

               float fps = ceil ( 1.0f/timeFPSTmp.asSeconds() );
               string tmpFSP  = std::to_string ( fps );
               char buffer [12];
               int cx;
               cx = snprintf ( buffer, 11, "FPS: %6.2f",fps );

	       
	       if (cx<=12)
	       {
		hud.textFPS.setString ( buffer ); 
	       } else 
	       {
		hud.textFPS.setString ( "snprintf error..." );  
	       }
	       

               hud.lastFPSUpdate.restart();
          }
